...
{
  // printf(1, "basicthreadmain starts!\n");
  int tid = (int) arg;
  int i;
...
basictest(void)
{
  ...
  for (i = 0; i < NUM_THREAD; i++){
      // printf(1, "retval add : %d\n", retval);
      ret = thread_join(threads[i], &retval);
      if(ret != 0) {
          printf(1, "panic at thread_join(ret)\n");
          return -1;
      }

      if((int)retval != i+1) {
          printf(1, "retval = %d, ", (int)retval);
          printf(1, "panic at thread_join(retval)\n");
          return -1;
      }
  }
  printf(1,"\n");
